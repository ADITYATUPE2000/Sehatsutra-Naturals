...
export async function POST(request){
    try {
        ...
        //resend eamil verification link
        if(!getUser.isEmailVerified){
            ...
            const token = await new SignJWT({userId: getUser._id.toString() })
             ...
             await sendMail('Email Verification request from Developer', email, emailVerificationLink(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/verify-email/${token}`))
             
             return response(false, 401, 'Your email is not verified. we have sent you a verification link to your registered email address.')
        }         
